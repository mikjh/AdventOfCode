...
function intersection(line1, line2)
    ...
    if min(line1.p1.x,line1.p2.x) > max(line2.p1.x,line2.p2.x) ||
        max(line1.p1.x,line1.p2.x) < min(line2.p1.x,line2.p2.x) ||
        min(line1.p1.y,line1.p2.y) > max(line2.p1.y,line2.p2.y) ||
        max(line1.p1.y,line1.p2.y) < min(line2.p1.y,line2.p2.y)

        return Point(0,0)
    end

    dx1 = line1.p1.x - line1.p2.x
    if dx1 == 0
...
function newPoint(p1, dir, steps)
    if dir == "U"
        return Point(p1.x,p1.y+steps)
    elseif dir == "R"
        return Point(p1.x+steps,p1.y)
    elseif dir == "D"
        return Point(p1.x,p1.y-steps)
    elseif dir == "L"
        return Point(p1.x-steps,p1.y)
...
@test newPoint(Point(0,0),"U",1) == Point(0,1)
@test newPoint(Point(0,0),"R",1) == Point(1,0)
@test newPoint(Point(0,0),"D",1) == Point(0,-1)
@test newPoint(Point(0,0),"L",1) == Point(-1,0)

function parse_to_lines(comma_separated_commands)
    commands = split(comma_separated_commands, ',')

    lines = Line[]
    p1 = Point(0,0)
    for command in commands
        m = match(r"(\w)(\d+)",command)
        dir = m[1]
        steps = parse(Int,m[2])
        p2 = newPoint(p1,dir,steps)
...
#basic parsing
@test parse_to_lines("U1") == Line[Line(Point(0,0),Point(0,1))]
@test parse_to_lines("R1") == Line[Line(Point(0,0),Point(1,0))]
@test parse_to_lines("D1") == Line[Line(Point(0,0),Point(0,-1))]
@test parse_to_lines("L1") == Line[Line(Point(0,0),Point(-1,0))]

#composed parsing
@test parse_to_lines("U1,R1") == Line[Line(Point(0,0),Point(0,1)), Line(Point(0,1),Point(1,1))]

function line_length(line)
    return abs(line.p2.x - line.p1.x) + abs(line.p2.y - line.p1.y)
end
...
function get_intersections(lines1, lines2)
    intersection_points = AggPoint[]
    ...
    for line1 in lines1
        ...
        for line2 in lines2
            ...
            if intersection_point != Point(0,0)
                length_to_intersection = point_distance(line1.p1,intersection_point) + point_distance(line2.p1,intersection_point)
                push!(intersection_points,AggPoint(intersection_point,length_to_intersection+line2_length+line1_length))
            end
            line2_length += line_length(line2)
        end
        line1_length += line_length(line1)
    end
    return intersection_points
end

function closest_intersection_length(comma_sep1, comma_sep2)
    intersections = get_intersections(parse_to_lines(comma_sep1),parse_to_lines(comma_sep2))

    return minimum(map(point->point.length,intersections))
end


@test closest_intersection_length("R8,U5,L5,D3", "U7,R6,D4,L4") == 30
@test closest_intersection_length("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83") == 610
@test closest_intersection_length("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7") == 410

println(closest_intersection_length(
